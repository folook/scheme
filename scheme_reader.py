...
def scheme_read(src):
    ...
    if src.current() is None:
        raise EOFError
    val = src.pop()  # src这个token列表已经改变了，弹出了最后一个值
    if val == "nil":
        return nil
    elif val not in DELIMITERS:  # 普通字母和普通数字开头
        return val
    elif val == "'":  # 引号开头
        "*** YOUR CODE HERE ***"
        return Pair('quote', Pair(scheme_read(src), nil))
    elif val == "(":  # 括号开头，调用read_tail
...
def read_tail(src):
    ...
    try:
        ...
        if src.current() == ")":  # # 左括号后是右括号
            src.pop()
            return nil
#        "*** YOUR CODE HERE ***"
        elif src.current() == '.':  # 左括号后是点，
            src.pop()  # 把点弹出
            first = scheme_read(src)  # 继续读后面的token
            second = read_tail(src)
            if second is not nil:
                raise SyntaxError("unexpected ')' after '.'")
            return first
        else:  # 左括号后是普通数字和字母
            # BEGIN PROBLEM 1
